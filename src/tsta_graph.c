...
static int tsta_graph_activate_sequence_node(tsta_node_t *node,
    ...
    if (!node->sorce) node->sorce = (char *)mm_malloc(state->L);
    if (!node->esorce) node->esorce = (char *)mm_malloc(state->L);
    if (!node->passing_seq)
        node->passing_seq = (unsigned char *)calloc(passing_bytes, 1);
    if (!node->edge_weight) node->edge_weight = (int *)malloc(sizeof(int));
    if (!node->f0) node->f0 = (char *)malloc(sizeof(char));
...
void free_node(tsta_node_t *n) {
    if (!n) return;
    free(n->prev); n->prev = NULL;
    free(n->next); n->next = NULL;
    mm_free(n->sorce); n->sorce = NULL;
    mm_free(n->esorce); n->esorce = NULL;
    if (n->source_store) {
    ...
    free(n->passing_seq); n->passing_seq = NULL;
    free(n->mismatch_node); n->mismatch_node = NULL;
    free(n->simple_sorce); n->simple_sorce = NULL;
    free(n->f0); n->f0 = NULL;
    free(n->edge_weight); n->edge_weight = NULL;
}
...
int tsta_msa_expand_passing_seq(tsta_graph_t *graph, size_t old_count,
                                size_t new_count) {
    if (!graph || new_count <= old_count) return -1;
    ...
    for (size_t i = 0; i < node_count; i++) {
        ...
        if (!node || !node->passing_seq) continue;
...
tsta_node_t *tsta_graph_build_initial(tsta_graph_t *p,
    ...
    /* Seed node */
    {
        ...
        if (!seed->sorce || !seed->esorce) goto fail;
        ...
        if (!seed->simple_sorce) goto fail;
    ...
    /* Head node */
    {
        ...
        if (!head->prev || !head->sorce || !head->esorce) goto fail;
        head->simple_sorce = NULL;
        head->prev[0] = seed_index;
        head->base = a[0];
        head->passing_seq = (unsigned char *)calloc(
            tsta_msa_passing_seq_bytes((size_t)sum), 1);
        if (!head->passing_seq) goto fail;
        ...
        head->mismatch_num = 0;
        head->id = head_index;
        if (!head->edge_weight || !head->f0) goto fail;
...
fail:
    ...
    free(p->unsort.data); p->unsort.data = NULL;
    p->unsort.length = p->unsort.capacity = 0;
    free(p->sort.data); p->sort.data = NULL;
    p->sort.length = p->sort.capacity = 0;
...
static inline int tp1_iterative(tsta_graph_t *s, tsta_node_index_t start,
    ...
    while (top > 0) {
        ...
        if (frame->stage == 0) {
            ...
            p->node_base_len =
                tsta_graph_node(s, p->prev[max_i])->node_base_len + 1;
        ...
        if (next->in_temp == 0 && next->mismatch_num == 0 && next->passing != 2) {
            if (top < stack_cap)
                stack[top++] = (tsta_dfs_frame){
                    .node = tsta_graph_node_index(s, next),
                    .stage = 0, .edge_index = 0};
            continue;
        }
        if (next->in_temp == 0 && next->mismatch_num > 0 && next->passing != 2) {
            int ready = 0;
    ...
    for (size_t i = 0; i < s->unsort.length; i++) {
        ...
        if (node->out == 0 && node->mismatch_num > 0) {
            for (int j = 0; j < node->mismatch_num; j++)
                if (tsta_graph_node(s, node->mismatch_node[j])->out != 0) s1 = 1;
            if (s1 != 1) node->passing = 2;
        }
        s1 = 0;
    }
        ...
        for (size_t i = 0; i < s->unsort.length; i++) {
            ...
            if (node->in_temp == 0) {
                if (node->mismatch_num == 0) {
                    subs = tp1_iterative(s, s->unsort.data[i], subs, stack,
                                         stack_cap);
                    break;
                } else {
                    for (int j = 0; j < node->mismatch_num; j++)
                        if (tsta_graph_node(s, node->mismatch_node[j])->in_temp ==
                            0)
                            c++;
                    if (c == node->mismatch_num) {
                        ...
                        for (int ss = 0; ss < node->mismatch_num; ss++)
                            if (tsta_graph_node(s, node->mismatch_node[ss])
                                    ->in_temp == 0)
                                subs = tp1_iterative(s, node->mismatch_node[ss],
                                                     subs, stack, stack_cap);
                        break;
                    }
                    c = 0;
                }
    ...
    while (1) {
        ...
        for (int i = max_i + 1; i < p->len; i++) {
            ...
            if (node->node_sorce >= 0 || node->node_status == 4) {
                    ...
                    if (prev->node_sorce >= 0) {
                        if (best_edge < node->edge_weight[j]) {
                            ...
                            best_i = j;
                                   ...
                                   tsta_graph_node(p, node->prev[best_i])
                                           ->node_sorce <= prev->node_sorce) {
                            best_edge = node->edge_weight[j];
                            best_i = j;
                        }
                    }
                ...
                node->node_sorce_source =
                    tsta_graph_node(p, node->prev[best_i])->sub;
                node->node_status = 0;
            }
        }
    }
                ...
                if (subs + s->last_node_num == s->len) {
                    for (size_t i1 = 0; i1 < s->unsort.length; i1++)
                        if (tsta_graph_unsort_node(s, i1)->in_temp == 0)
                            subs = tp_iterative(s, s->unsort.data[i1], subs,
                                                stack, stack_cap);
                }
                break;
            }
...
static void tsta_graph_setup_new_nodes(tsta_graph_t *n, tsta_node_t **seq,
                                       size_t base_nodes, size_t len_b,
...
static void tsta_graph_find_best_terminal(tsta_graph_t *n, int *num1) {
    int s1 = INT_MIN, s2 = 0;
    for (int i = n->len - 1; i > 0; i--) {
        ...
        if (s2 >= n->last_node_num) break;
    }
}

static int tsta_graph_handle_trace_match(tsta_graph_t *n, tsta_node_t *current,
                                         tsta_node_t **seq, int num2,
                                         int num, int sum, int *cont,
    ...
    *cont = 0;
    ...
    if (num2 > 0 && current->fsource_store &&
        NUM2(num2 - 1) > 0 &&
...
static void tsta_graph_connect_seq_to_existing(
    tsta_graph_t *n, tsta_node_t *current, tsta_node_t **seq,
    int num2, int num, int *cont, unsigned char *live_flags,
    size_t base_nodes, size_t len_b,
    tsta_node_index_t trace_parent_index,
    const tsta_msa_state *state) {
    if (num2 == 0) {
        if (*cont == 1 || *cont == 5) {
            if (live_flags[num2]) live_flags[num2] = 0;
            free_node(seq[num2]);
            seq[num2] = current;
        } else if (len_b > 1) {
        ...
        if (TRACE_SOURCE(prev, NUM2(num2 - 1)) / 42 == 1) {
            ...
            current->f0 = (char *)realloc(current->f0,
                                          (size_t)current->in * sizeof(char));
        ...
        tsta_node_t *prev = tsta_graph_node(n, parent_index);
        if (prev->sub != -1 &&
            TRACE_SOURCE(tsta_graph_node(n, prev->id), NUM2(num2 - 1)) / 42 == 1) {
            current->edge_weight[trace_parent_index]++;
        } else {
            current->in++;
            current->prev = (tsta_node_index_t *)realloc(
...
static int tsta_graph_handle_trace_mismatch(
    tsta_graph_t *n, tsta_node_t *current, tsta_node_t **seq,
    int num2, int num, int *cont, unsigned char *live_flags,
    size_t base_nodes, size_t len_b,
    tsta_node_index_t trace_parent_index, int *s2_flag,
    const tsta_msa_state *state) {
    int matched = 0;

    for (int s = 0; s < current->mismatch_num; s++) {
        tsta_node_t *mismatch = tsta_graph_node(n, current->mismatch_node[s]);
        if (seq[num2]->base != mismatch->base) continue;

        if (num2 != 0) {
            tsta_node_t *parent = tsta_graph_node(
                n, current->prev[trace_parent_index]);
            if (parent->sub != -1) {
                if (TRACE_SOURCE(parent, NUM2(num2 - 1)) / 42 == 1) {
                    for (int ss = 0; ss < mismatch->in; ss++) {
                        if (mismatch->prev[ss] ==
                            current->prev[trace_parent_index]) {
            ...
            if (*s2_flag != -1) {
                ...
                mismatch->edge_weight = (int *)realloc(
                    mismatch->edge_weight,
                    (size_t)mismatch->in * sizeof(int));
        ...
        if (matched == 1 && num2 != (int)len_b - 1) {
            seq[num2 + 1]->prev[seq[num2 + 1]->in - 1] =
                current->mismatch_node[s];
            mismatch->out++;
...
static int tsta_graph_create_mismatch_group(
    tsta_graph_t *n, tsta_node_t *current, tsta_node_t **seq,
    int num2, int num, int sum, unsigned char *live_flags,
    size_t base_nodes, tsta_msa_state *state,
    tsta_node_index_t trace_parent_index, size_t *new_node_count) {
    tsta_node_t *seq_node = seq[num2];
    ...
    for (size_t s = 0; s < mismatch_count; s++) {
        if (tsta_msa_mismatch_list_reserve(
                ...
                (size_t)tsta_graph_node(n, current->mismatch_node[s])
                        ->mismatch_num +
                    1) != 0)
            goto undo;
    }
...
undo:
    ...
    if (*new_node_count > 0) (*new_node_count)--;
    if (live_flags[num2]) live_flags[num2] = 0;
    free_node(seq_node);
    return -1;
}
...
tsta_graph_t *tsta_graph_update_impl(tsta_graph_t *n,
    ...
    if (!seq || !live_flags) goto fail_early;
    ...
    while (num1 != -1 && num2 != -1) {
        ...
        if (trace_type == 3) {
            int result = tsta_graph_handle_trace_match(
                n, current, seq, num2, num, sum, &cont, live_flags,
                base_nodes, state, (tsta_node_index_t)trace_parent_index,
                &new_node_count);
        ...
        if (trace_type == 1) {
            tsta_graph_connect_seq_to_existing(
                n, current, seq, num2, num, &cont, live_flags,
                base_nodes, len_b, (tsta_node_index_t)trace_parent_index,
                state);
