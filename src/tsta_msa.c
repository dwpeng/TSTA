...
static void
msa_workspace_reset(msa_thread_workspace* ws)
{
  if (!ws)
    return;
  if (ws->f_temp) {
...
static void
msa_workspace_destroy(void* ptr)
{
...
static int
msa_workspace_resize(msa_thread_workspace* ws,
                     int num,
                     int lane_len,
                     int block_width)
{
  ...
  if (ws->lane_len != lane_len || !ws->h_g) {
    mm_free(ws->h_g);
    ws->h_g = NULL;
    char* new_h_g = (char*)mm_malloc((size_t)lane_len);
    if (!new_h_g)
      return -1;
    ws->h_g = new_h_g;
  }
  ...
  for (int i = 0; i < ws->capacity_num; i++) {
    if (!ws->f_temp[i])
      ws->f_temp[i] = (char*)mm_malloc((size_t)block_width);
    if (!ws->VC2[i])
      ws->VC2[i] = (char*)mm_malloc((size_t)block_width);
    if (!ws->VC1[i])
      ws->VC1[i] = (char*)mm_malloc((size_t)block_width);
    if (!ws->r_s[i])
      ws->r_s[i] = (char*)mm_malloc((size_t)block_width);
...
static msa_thread_workspace*
msa_workspace_acquire(int num, int lane_len, int block_width)
{
...
static void
tsta_msa_apply_config(tsta_msa_aligner* aligner, const tsta_config* config)
{
...
static void
tsta_msa_cache_reset(tsta_msa_aligner* aligner)
{
  if (!aligner)
    return;
...
static int
tsta_msa_cache_reserve_slot(tsta_msa_aligner* aligner, size_t slot)
{
  if (!aligner)
    return -1;
...
/* ── Sequence packing for SIMD ─────────────────────────────────────── */

static char*
pack_sequence_for_simd(tsta_msa_aligner* aligner,
                       ...
                       tsta_msa_state* state)
{
  ...
  int required_capacity = state->length1 + 1;
  if (slot_entry->len < required_capacity) {
    mm_free(slot_entry->packed_seq);
    slot_entry->packed_seq = (char*)mm_malloc((size_t)required_capacity);
...
static int
tsta_msa_build_alignment_rows(tsta_graph_t* graph,
                              size_t sequence_count,
                              char*** rows,
                              size_t* aln_length)
{
...
static int
tsta_msa_final_score(tsta_graph_t* graph)
{
  int score = INT_MIN;
...
static int
tsta_msa_capture_result(tsta_graph_t* graph,
                        size_t sequence_count,
                        tsta_msa_result_t* result)
{
  ...
  if (!result)
    return -1;
  if (tsta_msa_build_alignment_rows(graph, sequence_count, &rows, &aln_length)
      != 0)
    return -1;
...
static void
tsta_msa_session_reset(tsta_msa_aligner* aligner)
{
  if (!aligner)
    return;
  if (aligner->pool) {
...
/* ── Core MSA SIMD block alignment (UNCHANGED) ─────────────────────── */

void
block_line_alignment(tsta_graph_t* graph,
                     int block_i,
                     int block_j,
                     int block_l,
                     tsta_node_t* row,
                     ...
                     msa_thread_workspace* ws)
{
  ...
  /* ── SIMD inner loop (unchanged) ── */
  __mxxxi zero, top, Smin, h, max, emax, eumax, source, source_num, esource,
      esource_num, fsource, s0, s1, s2, s3, s4, s5, s6, temp, temp1, temp2,
      mat, mis, egap, ogap, base, N, z;
  __mask mask, mask1, mask2, mask3, mask4, mask5, SN, SM, SX;
  __mxxxi y[pre_num], vc2[pre_num], vc1[pre_num], vc0[pre_num], diff[pre_num],
      t[pre_num], e[pre_num], eu[pre_num], ev[pre_num], f[pre_num],
      fv[pre_num], q[pre_num], v[pre_num], sum[pre_num];
  ...
  for (int i = 0; i < row->out; i++) {
    if (tsta_graph_node(graph, row->next[i])->sub >= kk
        && row->node_status != 3) {
      char* t_sorce = (char*)mm_malloc(state->length1);
      memcpy(t_sorce, row->sorce, state->L);
      mm_free(row->sorce);
      row->sorce = t_sorce;

      char* t_esorce = (char*)mm_malloc(state->length1);
      memcpy(t_esorce, row->esorce, state->L);
      mm_free(row->esorce);
      row->esorce = t_esorce;
...
void
tsta_msa_block_alignment(void* pa)
{
...
static int
tsta_msa_run_alignment_step(tsta_msa_aligner* aligner,
                            tsta_graph_t* graph,
                            const tsta_sequence_view_t* sequence,
                            int num,
                            int sum)
{
  ...
  state = &aligner->state;
  packed_sequence = pack_sequence_for_simd(aligner, (size_t)num, sequence,
                                           graph->len, state);
  if (!packed_sequence)
    return -1;
  ...
  for (unsigned int i = 0; i < tsl; i++) {
    ...
    for (int l = 0; l < j; l++) {
      ...
      if (tsta_threadpool_submit(aligner->pool, tsta_msa_block_alignment,
                                 &task, sizeof(task))
          != 0)
...
tsta_msa_aligner*
tsta_msa_aligner_create(const tsta_config* config)
{
...
void
tsta_msa_aligner_destroy(tsta_msa_aligner* aligner)
{
  if (!aligner)
    return;
...
int
tsta_msa_aligner_begin(tsta_msa_aligner* aligner,
                       ...
                       int initial_sequence_length)
{
  ...
  if (!aligner
      || tsta_sequence_view_make(&initial_view, initial_sequence,
...
int
tsta_msa_aligner_add(tsta_msa_aligner* aligner,
                     const char* sequence,
                     int sequence_length)
{
  size_t new_count;
  ...
  if (!aligner || !aligner->graph || !aligner->pool
      || aligner->sequence_count == 0
      || tsta_sequence_view_make(&sequence_view, sequence, sequence_length)
             != 0)
    return -1;
  ...
  if (tsta_msa_run_alignment_step(aligner, graph, &sequence_view,
                                  (int)aligner->sequence_count, (int)new_count)
      != 0) {
...
int
tsta_msa_aligner_get_result(tsta_msa_aligner* aligner,
                            tsta_msa_result_t* result)
{
...
int
tsta_msa_aligner_align(tsta_msa_aligner* aligner,
                       ...
                       tsta_msa_result_t* result)
{
  ...
  for (size_t i = 1; i < sequence_count; i++) {
    if (tsta_msa_aligner_add(aligner, sequences[i], sequence_lengths[i])
        != 0) {
      tsta_msa_session_reset(aligner);
      return -1;
    }
...
int
tsta_msa_align(const char* const* sequences,
               const int* sequence_lengths,
               size_t sequence_count,
