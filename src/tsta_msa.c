...
static int msa_workspace_resize(msa_thread_workspace *ws, int num, int lane_len,
    ...
    if (ws->block_width != block_width) {
        for (int i = 0; i < ws->capacity_num; i++) {
            mm_free(ws->f_temp[i]); ws->f_temp[i] = NULL;
            mm_free(ws->VC2[i]);   ws->VC2[i] = NULL;
            mm_free(ws->VC1[i]);   ws->VC1[i] = NULL;
            mm_free(ws->r_s[i]);   ws->r_s[i] = NULL;
        }
    }
...
static char *pack_sequence_for_simd(tsta_msa_aligner *aligner,
    ...
    if (slot_entry->len < required_capacity) {
        ...
        slot_entry->packed_seq =
            (char *)mm_malloc((size_t)required_capacity);
...
static void tsta_msa_free_node_traces(tsta_node_t *node) {
    if (!node) return;
...
static void tsta_msa_free_all_traces(tsta_graph_t *graph) {
    if (!graph || !graph->sort.data) return;
    for (int i = 0; i < graph->len; i++)
        tsta_msa_free_node_traces(tsta_graph_sort_node(graph, i));
}

static int tsta_msa_alloc_traces(tsta_graph_t *graph, size_t trace_length) {
    ...
    for (int i = 0; i < graph->len; i++) {
        ...
        if (!node->source_store || !node->esource_store ||
            !node->fsource_store) {
...
static void tsta_msa_release_alignment_work_buffers(tsta_graph_t *graph) {
    if (!graph || !graph->nodes.data) return;
...
static int tsta_msa_build_alignment_rows(tsta_graph_t *graph,
    ...
    if (!result_rows) return -1;

    for (size_t i = 0; i < sequence_count; i++) {
        result_rows[i] = (char *)malloc((size_t)graph->len + 1);
        if (!result_rows[i]) {
            for (size_t j = 0; j < i; j++) free(result_rows[j]);
            free(result_rows);
            return -1;
        }
...
static int tsta_msa_final_score(tsta_graph_t *graph) {
    int score = INT_MIN;
    if (!graph || !graph->sort.data) return 0;
    for (int i = 0; i < graph->len; i++) {
...
void block_line_alignment(tsta_graph_t *graph, int block_i, int block_j,
    ...
    if (!h_g || !f_temp || !VC2 || !VC1 || !r_s || !v0 || !vc_1 || !vc_2 ||
        !pd || !te) {
        ...
        free(v0); free(vc_1); free(vc_2); free(pd); free(te);
        return;
    }
...
void tsta_msa_block_alignment(void *pa) {
    ...
    int a1 = (((block_i - state->maxtag) > 0) * (block_i - state->maxtag) +
              block_l) * state->L;
