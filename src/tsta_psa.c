...
static void
psa_workspace_reset(psa_thread_workspace* ws)
{
  if (!ws)
    return;
  free(ws->maxsorce);
...
static int
psa_workspace_resize(psa_thread_workspace* ws, int lane_len, int block_width)
{
...
static inline void
blockmatrix_init(tsta_psa_state* state)
{
...
static int
tsta_psa_trace_context_alloc(tsta_psa_state* state)
{
  ...
  for (int i = 0; i < matrices->rows; i++) {
    matrices->back[i] = (char*)mm_malloc((size_t)matrices->cols);
    matrices->eback[i] = (char*)mm_malloc((size_t)matrices->cols);
    matrices->fback[i] = (char*)mm_malloc((size_t)matrices->cols);
      ...
      if (j - 1 >= 0
          && ((state->trace.matrices.fback[i][current] == 1
              ...
              || ((state->trace.matrices.fback[i][current] == 2
                   || state->trace.matrices.fback[i][current] == -2)
                  && state->trace.matrices
                             .fback[i][tsta_psa_trace_index(state, j - 1)]
                         < 0)))
...
/* ── Aligner alloc / lifecycle ─────────────────────────────────────── */

static void
tsta_psa_free_runtime_buffers(tsta_psa_aligner* aligner, int release_cache)
{
  if (!aligner)
    return;
...
static int
tsta_psa_ensure_runtime_buffers(tsta_psa_aligner* aligner,
                                size_t main_len,
                                size_t aux_len)
{
...
static void
tsta_psa_free_cached_sequences(tsta_psa_aligner* aligner)
{
  if (!aligner)
    return;
...
static int
tsta_psa_ensure_cached_sequence(tsta_psa_aligner* aligner,
                                int slot,
                                int padded_length,
                                ...
                                size_t sequence_length)
{
  ...
  if (!aligner || slot < 0 || slot >= 2 || !sequence_data || padded_length < 0)
    return -1;
                           ...
                           tsta_psa_state* state)
{
  ...
  if (tsta_psa_ensure_cached_sequence(aligner, 0, state->length[0],
                                      longer_data, longer_length)
      != 0)
    return -1;
  if (tsta_psa_ensure_cached_sequence(aligner, 1, state->length[1],
                                      shorter_data, shorter_length)
      != 0)
    return -1;
                                ...
                                tsta_psa_result_t* result)
{
  ...
  if (state->trace_enabled && tsta_psa_trace_context_alloc(state) != 0)
    goto cleanup;
