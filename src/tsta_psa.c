...
static inline void tsta_psa_block_alignment(void *p) {
    ...
    if (ws) {
        ...
        if (!maxsorce || !h_s || !rf || !t_temp || !e_temp || !q_temp ||
            !r_temp || !source[0] || !source[1] || !source[2])
            goto cleanup;
    }
                ...
                ((state->trace.matrices.fback[i][current] == 1 ||
                 ...
                 ((state->trace.matrices.fback[i][current] == 2 ||
                  ...
                  state->trace.matrices
                          .fback[i][tsta_psa_trace_index(state, j - 1)] < 0)))
                state->trace.matrices
                    .back[i][tsta_psa_trace_index(state, j - 1)] = 0;
            j--;
        } else {
            if (i - 1 >= 0 &&
...
static int tsta_psa_aligner_align_internal(
    ...
    status = (state->trace_enabled &&
              tsta_psa_capture_result(result, state) == 0)
                 ? 0
                 : -1;
